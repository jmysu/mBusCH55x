/*
        ...
        jimmy: 	-Change CH559 dataflash 1000
                                -Add CDC, LED, BOOT, SAVE, LOAD
                                -Remove SW;

    機能拡張 大まかな手順は、 (https://blog.goo.ne.jp/roboz80/e/3ecb59a9d2e60e16b5048879ef238924)
        １．Keyword tableにキーワードを追加する。
        ２．中間コードを追加する。
        ３．追加するコマンド/関数の処理を実装する。
        ４．追加したコマンド/関数を呼び出す処理を追加する。
...
#include "ttbasic.h"         //SIZE_LIST
#include "usb/cdc_device.h"  //isCDCavailable
...
// jimmy: move listbuf here to keep following memory clean
unsigned char listbuf[SIZE_LIST];  // リスト保存領域+ブートフラグ
short var[26];                     // 変数領域
short ary[SIZE_ARAY];              // 配列領域
// No memory declaration from here!
        ...
        if (*pkw != 0) {  // キーワードではなかった場合
            ...
            if (c_isdigit(*ptok)) {  // 数字の変換を試みる
                ...
                if (c_isalpha(*ptok)) {          // 変数だったら
                    if (len >= SIZE_IBUF - 2) {  // バッファに空きがない場合
                        err = ERR_IBUFOF;
                        return 0;
                    }
                    // バッファに空きがある場合
...
void putlist(unsigned char* ip) {
    ...
    while (*ip != I_EOL) {       // icodeバッファの末尾まで続ける
        if (*ip < SIZE_KWTBL) {  // キーワードの場合
            ...
            ip++;
        } else if (*ip == I_NUM) {  // 数値の場合
            putnum(getvalue(ip), 0);
            ip += 3;
            if (!IS_OP(*ip) && !IS_SEP(*ip)) c_putch(' ');
        } else if (*ip == I_VAR) {  // 変数の場合
            ip++;
            c_putch(*ip++ + 'A');
            if (!IS_OP(*ip) && !IS_SEP(*ip)) c_putch(' ');
        } else if (*ip == I_STR) {  // 文字列の場合
            ip++;
...
short getsize(void) {
    ...
    value = (short)listbuf + SIZE_LIST - lp - 1;
...
void iprint() {
    ...
    while (1) {
        ...
        if ((*cip == I_COMMA) || (*cip == I_SEMI)) {
            cip++;
            if (*cip == I_EOL)  // もし文末なら
                return;         // 終了
        } else {
            break;  // Not comma, Not semi
        }
    };
    newline();
}
        ...
        switch (*cip) {
            case I_STR:  // 文字列なら
                ...
                if (*cip == I_VAR) {  // 文字列のすぐ後ろが変数なら
                    ...
                    // if(err) return;//実験中
                    var[*cip++] = value;
                } else if (*cip == I_ARRAY) {  // 文字列のすぐ後ろが配列なら
                    cip++;
...
//===================================
// Command list,new,run,boot,save,load
void icom() {
    cip = ibuf;
    switch (*cip) {  // これらのコマンドはマルチステートメントで記述できません
        ...
        case I_BOOT:  // extend
            cip++;
            listbuf[SIZE_LIST - 1] = _BOOTMARK_FE;  // The last byte of list
            if (*cip == I_EOL) {
                flash_write(listbuf, SIZE_LIST);
                c_puts(" BOOTed:");
                putnum(listbuf[SIZE_LIST - 1], 3);
                c_puts("\n\r");
            } else
                err = ERR_COM;
            break;
        case I_SAVE:  // extend
            cip++;
            listbuf[SIZE_LIST - 1] = 0;  // The last byte
            if (*cip == I_EOL)
                flash_write(listbuf, SIZE_LIST);  // 0..SIZE_LIST
            else
                err = ERR_COM;
            break;
        case I_LOAD:  // extend
            cip++;
            if (*cip == I_EOL)
                flash_read(listbuf, SIZE_LIST);  // 0..SIZE_LIST
            else
                err = ERR_COM;
            break;
...
void basic(void) {
    ...
    // if (getBootflag() == I_BOOT) {
    if (getBootflag() == _BOOTMARK_FE) {  // Fix 0xFE
        c_puts(" BOOTRUN:");              // newline();
        flash_read(listbuf, SIZE_LIST);
        irun();
    }
    ...
    while (1) {
        ...
        Serial.printf("cdcbuf:%x ", cdcBuffer);
   Serial.printf("lbuf:%x ", lbuf);
        Serial.printf("lbuf:%s", lbuf);
        Serial.printf("LBUFz:%d\n\r", listbuf+SIZE_LIST);
   */
        if (err) {  // もし変換できなければ
            newline();
            c_puts("YOU TYPE: ");
