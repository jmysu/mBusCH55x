...
void (*runBootloaderF400)(void) = 0xf400;
//#define BOOT_LOAD_ADDR 0xF400

#define bSW_RESET         0x10      // software reset bit, auto clear by hardware
inline void RST_now(void) {
  SAFE_MOD    = 0x55;
  SAFE_MOD    = 0xAA; 
  RESET_KEEP  = 0; //!!!Force reset into APP
 ...
 */
  if (RESET_KEEP) { //1, entering bootloader
    RESET_KEEP = 0; //reset for next boot to APP
    Serial.print("\033[2J\033[H ***Bootloader***\a");
    delay(10);
    //runBootloader();
    //__asm
    //ljmp #BOOT_LOAD_ADDR
...
/*******************************************************************************
* Function Name  : GetChipUniqueID(void)
